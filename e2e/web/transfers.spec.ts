...
test.describe('Transfers', () => {
  test('create two accounts, transfer between them, verify both legs', async ({
    page,
  }) => {
    await page.goto('/');
    await page
    ...
    await expect(page.getByTestId('transfer-from-balance')).toHaveText(
      /Balance: \$0\.00/
    );
    ...
    await expect(page.getByText(`Transfer to ${TO_ACCT}`)).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.getByText(`Transfer from ${FROM_ACCT}`)).toBeVisible({
      timeout: 10000,
    });
