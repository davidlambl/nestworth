...
export function registerIpcHandlers(getAllowedOrigin: () => string | null): void {
  ipcMain.handle('csv:save', async (event, payload: { filename: string; csv: string }) => {
    ...
    if (typeof payload?.filename !== 'string' || typeof payload?.csv !== 'string') {
