...
- **A token refresh that fails and then succeeds within one read can trust an
  ...
  the parent's local synced splits and reports the pull complete, cursor banked. A later
  edit of that parent used to push by deleting every remote split and inserting none, a
  loss on every device; since #97 it goes up alone and the server keeps its splits, and
  the pull after that push brings them back here (unless the device clock runs ahead of
  the server's; see the clock-skew bullet above). The reconcile's split refresh does the
  same and banks its key. A flapped page in a multi-page reconcile enumeration truncates
  it, and the rows past it are absence-deleted locally until the next reconcile, up to a
  day later. All three
