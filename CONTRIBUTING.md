...
- **`pullChanges`**: Full-table pull for accounts/rules (`pullTableFull` -- reconciles deletions). Incremental pull for transactions (`pullTransactions` -- filters by `updated_at`, plus a full ID reconciliation pass to catch remote deletions).
- **Conflict resolution**: Last-write-wins. Local `ON CONFLICT DO UPDATE ... WHERE _sync_status = 'synced'` protects unsynced local edits from being overwritten by remote data.
...
- **`pullTransactions` reconciliation fetches all remote IDs every sync**: For users with very large transaction histories, this adds overhead. A soft-delete column on the Supabase `transactions` table would allow incremental deletion detection instead.
...
- **`expo-sqlite` web support**: Uses `sql.js` with IndexedDB/OPFS. Data durability on web is less guaranteed than native SQLite -- browser storage can be evicted.
