...
- **Mutation pattern**: Write to SQLite -> call `requestPush(user!.id)` -> invalidate relevant query keys in `onSuccess`. A write that touches more than one row goes in one `withTransactionAsync` (`transactionUpdate.ts`, `transferCreate.ts`, `transactionDelete.ts`, `usePostRecurringTransaction`, `useReorderAccounts`), so an interruption cannot leave half of it behind: `useDeleteTransaction` used to mark the splits and then the parent as separate statements, and an interruption in between left `deleted` splits under a live parent that no push could clear (#97). That holds for a crash or a failed statement, not for a collision with another transaction on the shared connection, which can end this one early and let its later statements commit alone (see `resetLocalData` above).
