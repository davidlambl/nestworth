...
- **Clock skew can briefly defer the _periodic_ reconcile for rows this device just
  ...
  none: the daily reconcile sees the drift and heals the transaction and its splits
  together. Until then -- at most a day, unless the realtime echo of its own upload
  reaches it after the mark, which can happen for an edit made longer than the skew
  before its push, and then the stale splits stay until the transaction next changes
  on the server -- its splits may still be stale, and an edit another device makes to
  it within the skew is refused by the pull's last-write-wins guard (this device's
  stamp is ahead) until that reconcile force-heals it. A push interrupted before the
  mark leaves the transaction pending for its next push, which uploads it again:
  marked synced any earlier, the realtime echo of its own upload could give it the
  server's stamp before its splits were written.
