...
- **Sync meta keys**: Scoped by userId (`last_pull_at:${userId}`, `last_pull_attempt_at:${userId}`, `last_txn_pull_at:${userId}`, `last_txn_reconcile_at:${userId}`) to support multi-user sign-in on the same device. `wipeLocalData` clears `sync_meta`, so a reset forces a fresh reconcile. `last_pull_at` is stamped only by a pull that read and trusted every table, and a pull that could not leaves it as it was, so "Last synced" is the time of the last COMPLETE pull; `last_pull_attempt_at` is stamped by every `pullChanges` that did not throw, complete or not, and `needsInitialPull` is true only while both are unset (#66).
