...
- The `QueryClient` has `MutationCache`/`QueryCache` `onError` handlers: a failed mutation is logged with its key (and, in development only, its variables, which carry amounts, payees and memos) and surfaced through `setLastError` (the label shows `Sync error`), since the mutation hooks have no `onError` of their own and `Alert.alert` is a no-op on web; a failed query is only logged, with its key. The sync engine reports through the same `setLastError`: a sync, bootstrap or reset that failed, a server missing a migration this build needs (`pushChanges`), and a pull that could not read or trust every table (#66).
