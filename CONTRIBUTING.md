...
- **`pullChanges`**: Full-table pull for accounts/rules (`pullTableFull` -- reconciles deletions). Self-healing pull for transactions (`pullTransactions`): an `updated_at > cursor` fast-path, then a reconcile pass that enumerates all remote `(id, updated_at)` and -- via the pure `planTransactionReconcile` -- deletes server-removed rows, pulls rows missing locally, and force-refreshes (`forceUpsertRemoteTransaction`) any `synced` row whose timestamp differs in *either* direction. This lets a device recover from a server correction whose `updated_at` is *older* than the device cursor (which the fast-path skips forever).
- **`resetLocalData`**: Recovery escape hatch -- flushes pending edits, wipes all local tables + the sync cursor (`wipeLocalData`), then re-runs `initialPull`. Surfaced in Settings as "Reset & re-download from cloud" for the rare case the local store drifts past what the reconcile can heal (e.g. an OPFS-backed SQLite file that "Clear site data" won't drop).
