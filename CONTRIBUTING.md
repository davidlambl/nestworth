...
- **Clock skew can briefly defer the _periodic_ reconcile for rows this device just
  ...
  clock.
  The opposite skew has a cost that does not self-correct (#97). A pending
  transaction whose splits this device did not change is pushed alone and adopts the
  server's `updated_at`. If the client clock runs ahead of the server's by more than
  the time since the previous pull began, that stamp is earlier than the cursor (the
  previous pull's start, in client time), so the next incremental pull does not list
  the transaction and never refreshes its splits, and the daily reconcile finds the
  local and server stamps equal. A stale or missing local copy of those splits --
  another device's re-split (#26), a download that failed, a transaction realtime
  delivered alone -- then stays until the transaction next changes on the server.
  The server's copy is right; the fix is to refresh the splits of transactions
  pushed alone right after the push.
...
- **A row created offline for an account another device deleted never syncs.** The
  ...
  nothing in the UI explains why it will not sync. The same hint-less `23503` is what a
