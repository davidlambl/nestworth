...
- **Clock skew can briefly defer the _periodic_ reconcile for rows this device just
  ...
  creation stamp, and the read finds no splits for it. After a failed read the push
  does not leave the transaction pending -- its edit is already on the server, and
  uploading it again could only overwrite a change another device made since -- but
  marks it synced WITHOUT the server's stamp, so the daily reconcile sees the drift
  and heals the transaction and its splits together. Until then, at most a day, its splits may
  still be stale, and an edit another device makes to it within the skew is refused
  by the pull's last-write-wins guard (this device's stamp is ahead) until that
  reconcile force-heals it.
